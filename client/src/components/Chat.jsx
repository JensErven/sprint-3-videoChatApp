...
const Chat = ({ setOpenChat, openChat, style, callDuration }) => {
  ...
  const { socket, sendMessage, communicatingWith, callingWith } =
  ...
  const handleSendMessage = (messageText) => {
    if (messageText !== "") {
      ...
      setMessage("");
      sendMessage(messageText.trim(), callingWith.id, communicatingWith); // Pass the message text to the sendMessage function
    }
  };
        ...
        >
          <div className="flex flex-row justify-between items-center p-4 ">
            ...
            <div className=" w-fit z-50 flex flex-row items-center justify-center gap-2">
              <p className="text-stone-200">{callingWith.name}</p>
